...
void IBShareLocation(const char *locationCharArray) {
	...
	NSString *locationString =
	    [NSString stringWithFormat:@"%s", locationCharArray];
	...
	CLLocationCoordinate2D coords = CLLocationCoordinate2DMake(
	        [[locationDict objectForKey:@"latitude"] floatValue],
	...
	CLLocation *location = [[CLLocation alloc]
	                        initWithCoordinate:coords
	                                  altitude:[[locationDict objectForKey:@"altitude"] floatValue]
	                        horizontalAccuracy:
	                        [[locationDict objectForKey:@"horizontalAccuracy"] floatValue]
	                        verticalAccuracy:
	                        [[locationDict objectForKey:@"verticalAccuracy"] floatValue]
	                        timestamp:myDate];

	[InfobipPush shareLocation:location withBlock: ^(BOOL succeeded, NSError *error)
	{
	    if (succeeded) {
	        UnitySendMessage([PUSH_SINGLETON UTF8String],
	                         [PUSH_SHARE_LOCATION UTF8String],
	                         [@"" UTF8String]);
		}
	    else {
